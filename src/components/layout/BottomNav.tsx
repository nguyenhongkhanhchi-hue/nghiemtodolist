...
import { CheckSquare, BarChart3, Sparkles, Settings, Trophy, FileText, DollarSign } from 'lucide-react';
...
const NAV_ITEMS: { page: PageType; icon: typeof CheckSquare; label: string }[] = [
  ...
  { page: 'ai', icon: Sparkles, label: 'AI' },
              ...
              <div className="relative">
                ...
                {page === 'tasks' && overdueCount > 0 && (
                  <div className="absolute -top-1.5 -right-2 size-4 rounded-full bg-[var(--error)] flex items-center justify-center">
                    <span className="text-[8px] font-bold text-white">{overdueCount > 9 ? '9+' : overdueCount}</span>
                  </div>
                )}
              </div>
